/**
 * Custom TipTap Node extensions for rich brain block types.
 *
 * Each non-text block is an `atom` node (non-editable) with a vanilla JS
 * NodeView. They participate in normal TipTap drag/reorder semantics
 * but their data is stored entirely in node attributes, so the server-side
 * ProseMirror ↔ markdown round-trip (`Magus.Brain.ProseMirrorProfile`) is
 * loss-free.
 *
 * Schema note: server-side block IDs are gone. We no longer maintain a
 * `blockId` attribute on any node — the markdown body is the single source
 * of truth and the editor hydrates from server-supplied ProseMirror JSON.
 */
  ...
  switch (variant) {
    ...
    default:
      return {
        icon: "ℹ️",
        label: "Note",
        cls: "bg-base-200/50 border-base-300/50",
      };
  }
}
...
export const SourceBlock = Node.create({
  ...
  renderHTML({ HTMLAttributes }) {
    return [
      "div",
      mergeAttributes(HTMLAttributes, { "data-type": "sourceBlock" }),
    ];
  },
  ...
  addNodeView() {
    return ({ node }) => {
      ...
      dom.className =
        "bg-base-200/50 border border-base-300/50 rounded-lg p-3 my-2 not-prose";
      ...
      const desc = a.description
        ? `<p class="text-xs text-base-content/60 mt-1">${escapeHtml(a.description)}</p>`
        : "";

      let badges = "";
      if (a.ingested) {
        badges += `<span class="text-xs bg-base-300/50 px-2 py-0.5 rounded text-primary">content extracted</span>`;
      }
      if (a.ingestionError) {
        badges += `<span class="text-xs bg-error/10 px-2 py-0.5 rounded text-error">extraction failed</span>`;
      }
      const link = a.url
        ? `<a href="${escapeHtml(a.url)}" target="_blank" rel="noopener" class="text-xs text-base-content/40 hover:text-primary ml-auto">open →</a>`
        : "";
      ...
      dom.innerHTML = `
        <div class="flex items-center gap-2 mb-1">
          ...
          <span class="text-xs text-base-content/40 ml-auto truncate max-w-[120px]">${host}</span>
        </div>
        ${desc}
...
export const FileBlock = Node.create({
  ...
  addNodeView() {
    return ({ node }) => {
      ...
      const dispatchOpen = () => {
        const tabRole =
          dom.closest("[data-brain-pane]")?.getAttribute("data-role") ||
          "primary";
      ...
      if (!file) {
        dom.className =
          "bg-base-200/50 border border-warning/30 rounded-lg p-3 my-2 flex items-center gap-3 not-prose";
        dom.innerHTML = `
          <div class="bg-warning/10 rounded-md w-9 h-9 flex items-center justify-center text-lg">⚠️</div>
          <div class="flex-1 min-w-0">
            <div class="text-sm font-medium text-base-content truncate">File no longer available</div>
            ${node.attrs.caption ? `<div class="text-xs text-base-content/40 truncate">${escapeHtml(node.attrs.caption)}</div>` : ""}
          </div>
        `;
        return { dom };
      }
      ...
      if (isImage) {
        ...
        dom.innerHTML = `
          ...
          />
          ${node.attrs.caption ? `<div class="text-xs text-base-content/60 mt-1">${escapeHtml(node.attrs.caption)}</div>` : ""}
          <button
            type="button"
            data-open-full
            class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition bg-base-100/80 rounded p-1 text-xs"
        ...
        dom.className =
          "bg-base-200/50 border border-base-300/50 rounded-lg p-3 my-2 flex items-center gap-3 not-prose cursor-pointer hover:bg-base-200/70";
        dom.innerHTML = `
          <div class="bg-base-300/50 rounded-md w-9 h-9 flex items-center justify-center text-lg">${fileIcon(file)}</div>
          <div class="flex-1 min-w-0">
            <div class="text-sm font-medium text-base-content truncate">${escapeHtml(file.name || "")}</div>
            <div class="text-xs text-base-content/40">${escapeHtml(file.mime_type || "")} · ${formatBytes(file.file_size)}</div>
            ${node.attrs.caption ? `<div class="text-xs text-base-content/60 truncate">${escapeHtml(node.attrs.caption)}</div>` : ""}
          </div>
...
export const MessageBlock = Node.create({
  ...
  renderHTML({ HTMLAttributes }) {
    return [
      "div",
      mergeAttributes(HTMLAttributes, { "data-type": "messageBlock" }),
    ];
  },
  ...
  addNodeView() {
    return ({ node }) => {
      const dom = document.createElement("div");
      dom.className =
        "bg-base-200/50 border-l-[3px] border-l-primary rounded-r-lg p-3 my-2 not-prose";
      ...
      dom.innerHTML = `
        <div class="flex items-center gap-2 mb-1">
          <span class="text-xs bg-primary text-primary-content px-1.5 py-0.5 rounded font-medium">from conversation</span>
        </div>
        <p class="text-sm text-base-content leading-relaxed">${preview}</p>
      `;
...
export const CalloutBlock = Node.create({
  ...
  renderHTML({ HTMLAttributes }) {
    return [
      "div",
      mergeAttributes(HTMLAttributes, { "data-type": "calloutBlock" }),
    ];
  },
    ...
    return ({ node }) => {
      ...
      const agentBadge =
        a.contributorType === "custom_agent"
          ? `<span class="text-xs text-base-content/40 ml-auto">by agent</span>`
          : "";
      ...
      dom.innerHTML = `
        ...
        <p class="text-sm text-base-content leading-relaxed">${renderInlineMarkdown(a.text)}</p>
      `;
...
export const ImageBlock = Node.create({
  ...
  addNodeView() {
    return ({ node }) => {
      ...
      const pageId = editorRoot?.getAttribute("data-page-id");
      const fileMap =
        (pageId && (window.__brainFileMaps || {})[pageId]) || {};
      const file = node.attrs.fileId ? fileMap[node.attrs.fileId] : null;
      const url = file ? fileImageUrl(file) : "";

      const caption = node.attrs.caption
        ? `<p class="text-xs text-base-content/60 mt-1 px-1">${escapeHtml(node.attrs.caption)}</p>`
        : "";

      if (url) {
        dom.innerHTML = `
          <div class="rounded-lg overflow-hidden border border-base-300/50">
