...
Each conversation has a `ConversationAgent` (id `conv:<uuid>`) managed by `Jido.Agent.InstanceManager`. Uses `ReactStrategy` for the ReAct loop and composable plugins to translate strategy signals into PubSub + DB writes. Agents hibernate to Postgres after 5 min without an attachment and thaw on next signal; an in-flight run holds an attachment (the runtime task attaches to the parent), so idle timeout can never kill a working turn, and the SPA channel warm-boots the agent on join. Long turns stay observable via a `turn.keepalive` ticker (`Magus.Agents.Support.TurnKeepalive`, also used by media generation) that touches AgentRun heartbeats. Turns interrupted anyway (deploy/crash) are settled at checkpoint time (streaming rows swept + visible `turn_interrupted` event) and retried on thaw by `Magus.Agents.Recovery` with a "don't redo completed work" context note. Resource stops (per-run token budget, `max_iterations`) end with one tool-less wrap-up LLM call instead of dropping the work; transient LLM errors retry with backoff. Loop tuning knobs live under `config :magus, :agents` (see config.exs).
