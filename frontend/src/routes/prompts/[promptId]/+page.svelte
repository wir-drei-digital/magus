<script lang="ts">
	...
	// ?edit=true deep links open straight into the edit form (classic parity).
	let editing = $state(false);
	let editParamApplied = false;
	$effect(() => {
		if (editParamApplied || !prompt) return;
		editParamApplied = true;
		if (page.url.searchParams.get('edit') === 'true') editing = true;
	});
	let saving = $state(false);
	let saveError = $state<string | null>(null);

	// Form fields (mirrors the classic edit form's core; model preset and
	// language stay deferred).
	let name = $state('');
	let content = $state('');
	...
	$effect(() => {
		...
		// Creation lives in the shared "New prompt" dialog; this route only edits.
		if (id === 'new') {
			void goto(`${base}/prompts`, { replaceState: true });
			return;
		}
	...
	function syncForm(data: PromptDetail) {
		name = data.name;
	...
	async function save() {
		...
		if (result.success) {
			...
			saveError = result.errors[0]?.message ?? 'Prompt could not be saved';
		}
	}

	function cancelEdit() {
		if (prompt) syncForm(prompt);
		editing = false;
		saveError = null;
	}
...
<div class="flex h-full min-h-0 flex-col" data-testid="prompt-detail">
	...
	{:else if editing}
		...
		<div class="wb-scroll mx-auto w-full max-w-2xl min-h-0 flex-1 overflow-y-auto p-6">
			<Section title="Edit prompt" description="Update this prompt and save your changes.">
				<form
				...
				>
					...
					<Field label="Type">
						<select bind:value={type} class="{CONTROL_CLASS} w-48">
					...
					<Field label="Additional information">
						<textarea bind:value={additionalInformation} rows="3" class={TEXTAREA_CLASS}></textarea>
					</Field>
					...
					<div class="flex items-center gap-2 pt-1">
						<Button type="submit" disabled={saving || !canSave} data-testid="prompt-save">
							{saving ? 'Saving…' : 'Save'}
						</Button>
	...
	{:else}
		...
		<header class="flex shrink-0 items-center gap-2.5 border-b py-3 pr-6 pl-14 md:pl-4">
			<button
				...
				onclick={() => void goto(`${base}/prompts${page.url.search}`)}
			>
