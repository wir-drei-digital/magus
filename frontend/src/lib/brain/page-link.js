 ...
 *
 * @param {any} pages - Brain pages for the popup: a static `{id, title}[]` array
 *   or a live getter `() => {id, title}[]` (so the list can update without
 *   re-creating the editor).
 * @returns {Extension}
 */
export function createPageLink(pages = [], { onPageRefClick } = {}) {
	// Accept a static array or a getter, so the host can keep the page list live
	// (it loads asynchronously) without re-creating the editor.
	const getPages = typeof pages === 'function' ? pages : () => pages;

	return Extension.create({
		name: 'pageLink',

		addOptions() {
			return { getPages, onPageRefClick };
		},

		addProseMirrorPlugins() {
			const getExtensionPages = this.options.getPages;
			...
			return [
				...
				createPageRefDecoPlugin(getExtensionPages, this.options.onPageRefClick)
			];
		}
	});
}
...
function createPageRefDecoPlugin(getPages, onPageRefClick) {
	let lastPages = getPages();

	return new Plugin({
		key: pageLinkDecoKey,
		state: {
			init(_, state) {
				return buildDecorations(state.doc, getPages());
			},
			apply(tr, old) {
				if (tr.getMeta(pageLinkDecoKey) === 'rebuild' || tr.docChanged) {
					return buildDecorations(tr.doc, getPages());
				}
				return old;
			}
		},
		...
		view() {
			return {
				update(view) {
					// Rebuild ref decorations when the page list reference changes
					// (it loads async in the host; a new array means new titles).
					const next = getPages();
					if (next !== lastPages) {
						lastPages = next;
						const { tr } = view.state;
