...
import type { PlanTask, PageTreeNode, TaskEventEntry, TaskEventKind } from '$lib/ash/api';
...
beforeEach(() => {
	...
	brainPages.mockResolvedValue(
		ok([
			pageNode({ id: 'plan-a', title: 'Launch plan' }),
			pageNode({ id: 'plan-b', title: 'Research plan' })
		])
	);
});

/** A brain page fixture, for the plan-title lookup. */
function pageNode(overrides: Partial<PageTreeNode> & { id: string }): PageTreeNode {
	return {
		title: 'Plan',
		icon: null,
		parentPageId: null,
