<script lang="ts">
	...
	// The gallery (prompts/+layout.svelte) is the browse surface now; this rail is
	...
	const scopes = $derived([
		...
		...(session.user?.currentWorkspaceId
			? [
					{ key: 'shared', label: 'Shared', icon: Users, count: promptsNav.shared.length },
					{ key: 'personal', label: 'Personal', icon: User, count: promptsNav.personal.length }
				]
			: [])
	]);
	...
	const tags = $derived.by(() => {
		...
		for (const prompt of allPrompts) {
			for (const tag of prompt.tags) counts.set(tag.name, (counts.get(tag.name) ?? 0) + 1);
		}
	...
	const inPrompts = $derived(page.url.pathname.startsWith(`${base}/prompts`));

	const scopeHref = (key: string | null) =>
		key ? `${base}/prompts?scope=${key}` : `${base}/prompts`;
	const tagHref = (name: string) => `${base}/prompts?tag=${encodeURIComponent(name)}`;
</script>

<div data-testid="prompts-nav" class="contents">
	{#if promptsNav.loading && allPrompts.length === 0}
		<Sidebar.Group>
	...
	{:else}
		...
		{#if tags.length > 0}
			<Sidebar.Group data-testid="prompts-tags-nav">
				...
				<Sidebar.GroupContent>
					<Sidebar.Menu>
						{#each tags as tag (tag.name)}
							<Sidebar.MenuItem>
								<Sidebar.MenuButton
									isActive={inPrompts && activeTag === tag.name}
									data-testid="prompts-tag"
								>
									{#snippet child({ props })}
