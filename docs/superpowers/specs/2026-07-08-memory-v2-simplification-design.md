...
- **Eval-gated:** the LongMemEval-S benchmark (`Magus.Eval.Runner` with `Magus.Eval.Benchmarks.LongMemEval`, limit 18, per the procedure in `docs/superpowers/plans/2026-07-04-memory-eval-baselines.md`) before and after the Phase 2 removals; the 3/18 hardened baseline must not regress (this specifically watches the association-layer and promotion removals). The distiller suite `test/magus/agents/actions/distill_user_profile_test.exs` must stay green and gains a case covering the new local-memory input window. Yardstick note: LongMemEval measures cross-session episodic recall, which is the Super Brain's long-term job, not this layer's; it stays here as a regression tripwire, while the profile eval and the trust invariants below are this layer's primary quality metrics.
